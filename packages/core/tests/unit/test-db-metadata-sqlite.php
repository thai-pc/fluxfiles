...
/**
 * Behavioral test suite for FluxFiles\Db\DbMetadataHandler over a temp SQLite
 * file — the `db` storage backend's metadata/index/trash/audit store.
 *
 * Usage:
 *   php tests/unit/test-db-metadata-sqlite.php
 */
...
$dbFile = '/tmp/ff_test_db_metadata_' . getmypid() . '.sqlite3';
$storageRoot = '/tmp/ff_test_db_metadata_storage_' . getmypid();
...
echo "\n{$cyan}╔══════════════════════════════════════════════════╗{$reset}\n";
echo "{$cyan}║      DbMetadataHandler (SQLite) Test Suite        ║{$reset}\n";
echo "{$cyan}╚══════════════════════════════════════════════════╝{$reset}\n\n";
...
test('purgeAuditBefore() removes old rows and archives_deleted is always 0', function () use ($repo, $disk) {
    ...
    // Directly age the row so it falls before the purge cutoff.
    $conn = null;
