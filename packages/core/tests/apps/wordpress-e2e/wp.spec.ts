...
import { pngFile, largePngFile, cardByName, uploadFile } from '../e2e/helpers';
...
// Phase 6 coverage: setup.sh gates the local disk (fluxfiles_local_private=1), so
// FileManager::list() emits gated /api/fm/stream URLs (file.url / variant urls)
// instead of a static path, and img_base/img_srcset drive /api/fm/img — both
// proxied through the WP REST API (handleStream/handleImg in FluxFilesApi.php).
...
test('wordpress proxy: gated media stream + on-demand img are served through the REST API', async ({ page }) => {
  await loginAdmin(page);
  await page.goto(`${BASE}/files/`);
  ...
  const name = `wp-stream-${Date.now()}.png`;
  ...
  expect(streamSrc).toMatch(/\/wp-json\/fluxfiles\/v1\/api\/fm\/stream\?token=/);
  ...
  expect(imgUrl).toMatch(/\/wp-json\/fluxfiles\/v1\/api\/fm\/img\?token=/);
