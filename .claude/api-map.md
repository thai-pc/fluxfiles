...
- Per-tenant config is carried in **JWT claims**, not a `/config` route — the token *is* the config. **[`docs/CONFIG.md`](../docs/CONFIG.md) is the single source of truth for all 81 claims + env vars** (grouped: access, upload, images/WebP/AVIF, watermark, media, optimize, terminal, usage, import, paid-gates). Mint them in one options object: `fluxfiles_token(['user'=>…, 'claims'=>[…]])`, where `claims` is the escape hatch for any claim by its raw name (same passthrough in node/laravel/wordpress).
