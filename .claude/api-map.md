...
- `POST /api/fm/extract {disk, path, dest?}`
  - Extracts a `.zip` in place (default `dest` = a folder named after the archive). Two passes: structural/path/size/quota checks run before writing; scanning and storage writes run per entry. **Not transactional:** a scan or storage failure can leave earlier successful entries in place. Write perm + `allow_extract`. Guards: **zip-slip** (absolute/`..`/drive-letter → 403 `zip_slip`), dangerous-ext (`assertSafeFilename`) + `allowed_ext`, system-path skip, **zip-bomb** (uncompressed size + count caps → 413), **quota** and **max_files** before writing. Returns JSON → **proxied** by Laravel/WordPress.
...
- `POST /api/fm/share {disk, path, ttl?, label?, password?, max_downloads?}` · `GET /api/fm/share/list?disk=` · `POST /api/fm/share/revoke {disk, jti}` · `GET /api/fm/share/analytics?disk=&jti=&limit=&offset=&event=` — **paid** (`share`, 3-layer gate incl. `allow_share`)
  - Operator side of the landing above. Create returns `{token, jti, expires, label, max_downloads, has_password, url}` — the **token is returned once and never stored**, so a listed share can only be revoked, not re-linked; `url` comes from the `share_base_url` claim, else the request origin + `/public/share.html`. The record (`<store>/_fluxfiles/shares.json`) carries `store`/`owner`/`label`/`url_ttl`/`preview`; `list`/`revoke` are owner-filtered under `owner_only`. Claims: `share_url_ttl`, `share_base_url`, `share_preview` (all read at create time and baked into the record, so a public request needs none). `analytics` returns the per-event log (`view`/`download`/`unlock_fail`, timestamp/IP/user-agent) — populated only for shares created with `share_analytics` on (baked in at create time, like `share_url_ttl`); `limit` (1–500, default 100) + `offset` + an optional `event` filter, owner-scoped the same way as `revoke`. Core-standalone — not proxied by the adapters.

- `POST /api/fm/intake {disk, path(dir), ttl?, label?, password?, max_files?, max_mb?, allowed_ext?}` · `GET /api/fm/intake/list?disk=` · `POST /api/fm/intake/revoke {disk, jti}` · `GET /api/fm/intake/analytics?disk=&jti=&limit=&offset=&event=` — **paid** (`intake`, 3-layer gate incl. `allow_intake`)
  - Operator side of the Upload Portals (`/public/intake.html` + the pre-auth `intake/info|upload`). Create mints a **write-only** portal token scoped to the folder and returns `{token, jti, expires, label, max_files, max_mb, allowed_ext, has_password, url}` — same one-shot posture as Share (never stored, revoke-only), and `url` comes from the `intake_base_url` claim, else the request origin + `/public/intake.html`. The record (`<store>/_fluxfiles/intakes.json`) carries `store`/`owner`/caps/`received`; `list`/`revoke` are owner-filtered under `owner_only` (a legacy record without `owner` fails **closed**), and revoke writes a **tombstone** so an interleaved upload's counter write can't resurrect the portal. `analytics` returns the per-event log (`received`/`rejected`, timestamp/IP/user-agent/reason/filename) — populated only for portals created with `intake_analytics` on; same `limit`/`offset`/`event` params and owner-scoping as Share's analytics above. Core-standalone — not proxied by the adapters.
...
- `POST /api/fm/hold {disk, path, reason?}` · `POST /api/fm/hold/release {disk, hold_id, reason?}` · `GET /api/fm/hold/list?disk=&include_released=` — **paid** (`legal-hold`, 3-layer gate; `docs/design/RETENTION-LEGAL-HOLD-DESIGN.md` §2)
