...
| `webhook_secret` | string | — | HMAC signing secret (`X-FluxFiles-Signature`). Empty = a key **derived** from `FLUXFILES_SECRET` (HMAC-SHA256 with a fixed info string, `WebhooksModule::effectiveSecret()`) — never the raw server secret, since that also signs the main auth JWTs and the webhook receiver sees the signature by design. |
| `allow_ai_vision` | bool | `false` | AI Vision (BYO-key): bg-remove/upscale/smart-crop, with an operator UI (detail panel + context menu + action sheet). Gates `POST /api/fm/ai-vision`. Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.39). |
| `allow_ocr` | bool | `false` | OCR. Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.39). |
...
| `allow_backup` | bool | `false` | Backup Bridge. Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.39). |
| `allow_c2pa` | bool | `false` | C2PA provenance (Enterprise). Gates `POST /api/fm/c2pa` (verify) and `POST /api/fm/c2pa/sign`. Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.61 for `c2pa/sign`). |
| `allow_audit_export` | bool | `false` | Audit Export (Enterprise). Gates `GET /api/fm/audit/export` (streams NDJSON/CSV) and `POST /api/fm/audit/purge` (the latter also requires an unscoped/admin token — see below). Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.77). |
...
| `allow_dlp_scan` | bool | `false` | DLP / PII detection on write (Enterprise). Scans **upload**, **code-editor save**, and **each zip-extract entry** before the bytes are written; a detected entity above `dlp_min_score` → `422 pii_detected` (audited as `dlp_blocked`), nothing stored. **Fail-closed** like `allow_virus_scan`: with the claim on, a missing/unlicensed module or an unreachable engine refuses the write (`501`/`402`/`403`) rather than storing unscanned. Engine: self-hosted [Microsoft Presidio](https://microsoft.github.io/presidio/) (`FLUXFILES_DLP_ENDPOINT`); 501 `dlp_unconfigured` when unset. **Not covered:** S3 multipart chunk upload — `POST /api/fm/chunk/*` returns `409 dlp_unscannable` while this claim is on, since those bytes go browser→S3 and can never be scanned. Never leaks raw matched text/offsets/counts — only deduped entity-type names reach the error response and the audit log. Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.82). |
| `dlp_entity_types` | string[] or null | _(engine default)_ | Allowlist of Presidio entity types (e.g. `US_SSN`, `CREDIT_CARD`, `EMAIL_ADDRESS`) that trigger a block. Uppercased, `[A-Z_]+`-filtered; unset/empty = the engine's own full default set. |
| `dlp_scan_extensions` | string[] | `txt,csv,tsv,json,ndjson,log,md,yaml,yml,xml,html,htm,sql` | Extensions eligible for scanning (dot/case stripped). Anything else is **skipped, not blocked** — checked in core before the module is invoked, so an engine outage never blocks an ineligible upload (e.g. a `.jpg`). |
| `dlp_max_scan_kb` | int (KB) | `2048` | Files larger than this are **skipped, not blocked** (same eligibility pre-filter as above). Clamped `[16, 51200]`. `0` = default. |
| `dlp_min_score` | float | `0.6` | Minimum Presidio confidence score (0–1) for a match to count as a detection. Clamped `[0, 1]`. `0` = default. |
| `allow_legal_hold` | bool | `false` | Legal hold / retention (Enterprise) — gates **placing/releasing** a hold: `POST /api/fm/hold` (place, requires `audit` perm + a non-empty `reason`), `POST /api/fm/hold/release`, `GET /api/fm/hold/list`. **Enforcement is free/core and license-independent**: once a hold exists, `delete`/`trash`/`rename`/`move`/`cross-move`/`trash/purge`/`trash/empty` on the held path (or an ancestor/descendant of it) are blocked with `403 legal_hold_active` even if this module is uninstalled or unlicensed — see `FileManager::assertNoActiveHold()`. Visibility (`on_hold` in `/list`, `GET /api/fm/hold/status`) is also free/core, gated only by the ordinary `read` perm; the hold's `reason`/`placed_by`/`placed_at` are additionally hidden from a caller without the `audit` perm. `copy`/`cross-copy` are NOT blocked (a hold protects the original, not derivative copies). Proxied by the Laravel/WordPress adapters too (requires core ≥ 0.2.82). |
...
| `FLUXFILES_STORAGE_PATH` | — | Runtime-state dir for server-local state that is NOT per-disk (rate-limit counter, SFTP `ControlMaster` sockets, OIDC discovery cache) — defaults to `packages/core/storage/`. **Not** the local disk root, which is a separate, hardcoded `config/disks.php` path (`storage/uploads/`). |
| `FLUXFILES_STORAGE_BACKEND` | `json` | `json` (unchanged default — metadata/search/folder-index/audit/trash live per-disk in `_fluxfiles/*.json`, which travels with user storage) or `db` (that per-disk data lives in the configured database instead — one switch drives `MetadataRepositoryInterface`). **Rate limits are the one exception**: they always live server-side in a single `rate_limit.json` at `FLUXFILES_STORAGE_PATH` (not per-disk, not under `_fluxfiles/`) when `json`, or via `RateLimiterStorageInterface`'s DB implementation when `db` — this switch affects both. Quota **enforcement** always uses a live storage scan regardless of this setting. |
...
| `FLUXFILES_LEGAL_HOLD_MAX_ACTIVE` | `1000` | Hard cap on simultaneously active (non-released) legal holds per disk (paid). Not a JWT claim — a system-integrity cap, not a per-tenant business knob, same reasoning as `versioning_max`'s hard cap. `0` disables the cap. |
