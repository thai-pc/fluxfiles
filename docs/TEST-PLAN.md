...
- ✅ **Browser e2e (Playwright)** `packages/core/tests/browser/` (14): boots the real PHP server, drives the standalone UI in chromium. `ui.spec.ts` (2): valid token → file manager renders; no token → auth screen. `interactions.spec.ts` (12): upload progress UI (held request → spinner + current filename + `(n/2)` count visible); upload via file input → card appears; create folder + breadcrumb navigation; search narrows the grid; dark-mode toggle adds/removes `.dark`; select+confirm delete removes the card; inline crop tab → "Save as Copy" produces `{base}_cropped.{ext}`; picker mode (iframe host page) FM_READY→FM_CONFIG handshake → double-click emits a single `FM_SELECT`; picker `multiple:true` → Ctrl/Cmd-click several files + "Select N items" emits an `FM_SELECT` **array**; bulk delete (Ctrl/Cmd-click N files → confirm → all removed); bulk move (select N → "Move" modal → quick-pick subfolder → files land in `dest`); bulk download (select N → "Download" → one download per file).
