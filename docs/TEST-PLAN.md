...
- ✅ **Browser e2e (Playwright)** `packages/core/tests/browser/` (17): boots the real PHP server, drives the standalone UI in chromium. `ui.spec.ts` (2): valid token → file manager renders; no token → auth screen. `interactions.spec.ts` (15): upload progress UI (held request → spinner + current filename + `(n/2)` count visible); upload via file input → card appears; `maxUploadMb` blocks an oversized file client-side (toast, no API call); `maxFiles` caps an oversized drop batch; drop outside the dropzone uploads without navigating; create folder + breadcrumb navigation; search narrows the grid; dark-mode toggle adds/removes `.dark`; select+confirm delete removes the card; inline crop tab → "Save as Copy" produces `{base}_cropped.{ext}`; picker mode (iframe host page) FM_READY→FM_CONFIG handshake → double-click emits a single `FM_SELECT`; picker `multiple:true` → Ctrl/Cmd-click several files + "Select N items" emits an `FM_SELECT` **array**; bulk delete/move/download.
...
- ✅ **Max files** (`max_files` claim): upload to limit then next → 413 `too_many_files`; `0` = unlimited; `getFileCount` skips `_fluxfiles/`/`_variants/` — `test-max-files.php` (3).
